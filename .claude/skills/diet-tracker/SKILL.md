---
name: diet-tracker
description: 專業營養追蹤與健康顧問。追蹤每日飲食、深度分析營養素、提供基於 2025 營養科學的專業建議。支援指令：add（記錄）、today（今日統計）、week（週報）、suggest（智慧建議）、analyze（深度分析）、score（健康評分）、learn（營養知識）、goal（設定目標）、calendar（行事曆首頁）。
---
