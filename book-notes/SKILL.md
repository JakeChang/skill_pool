...
**正確的檔案位置：**
- 儲存位置：`book-notes/{book-slug}.json`
- HTML 輸出：`book-notes/reports/`
...
1. 詢問書名和作者
2. 建立 `book-notes/{slug}.json`，設定 `status: "reading"`
...
使用 Glob 讀取 `book-notes/*.json`，列出書籍與狀態。

### 生成 HTML

1. 讀取 [assets/html-templates.md](assets/html-templates.md) 取得模板
2. 用書籍資料填充模板變數
3. 輸出到 `book-notes/reports/` 目錄：
   - `index.html` - 書籍列表
   - `{book-id}.html` - 書籍詳情
